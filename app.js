'use strict';
const fs = require('fs');//fsはファイルを扱うためのモジュール
const readline = require('readline');//readlineはファイルを一行ずつ読み込むためのモジュール
const rs = fs.ReadStream('./popu-pref.csv');//（）内のファイルからファイルの読み込みを行うStreamを作成する。streamとは非同期の情報を扱うための概念。
const rl = readline.createInterface({ 'input': rs, 'output': {} });//rsをreadlineオブジェクトのinputとして設定し、rlオブジェクトを作成する。
const map = new Map();//key:都道府県　value:集計データのオブジェクト

rl.on('line', (lineString) => {//rlオブジェクトでlineというイベントが発生したら無名関数を呼び出す

    const columns = lineString.split(',');//lineStringで与えられた文字列をカンマで分割して、それをcolumnsという配列にする
    const year = parseInt(columns[0]);
    const prefecture = columns[2];
    const popu = parseInt(columns[7]);//配列となったcolumnsから並び順の番号にアクセスしてそれぞれの変数に保存
...
rl.resume();//resumeメソッドを呼び出し、ストリームに情報を流し始める。
